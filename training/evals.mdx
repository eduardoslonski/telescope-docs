...
This means any training environment can also be used as an eval by referencing its name. Dedicated evals exist for benchmarks that have no training environment (e.g., `math500` uses the MATH-500 test set but wraps the `hendrycks_math` environment for answer verification).
