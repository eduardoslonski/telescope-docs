...
Telescope supports training models that use tools during rollouts. Two approaches are available: **environment-level** tool calling via the `ToolEnvironment` base class, and **inference-level** tool calling via vLLM's built-in parser. You can use either or both depending on your needs.
...
These are useful both for reward computation (e.g., penalizing excessive tool use) and for monitoring via `sample_metrics`. See [Reward Design](/training/reward-design) for patterns on incorporating tool metrics into rewards.

## vLLM tool calling

For models that support native tool calling (e.g., Hermes-format models), you can enable vLLM's built-in parser instead of or alongside environment-level parsing:
...
When enabled, vLLM parses tool calls from model output using the specified format and returns structured tool call objects. This is useful when:
- The model was pretrained with a specific tool call format (Hermes, etc.)
- You want tool call parsing handled at the inference level rather than in environment code

### Thinking mode with tools

You can combine thinking mode and tool calling:
...
The model generates `<think>` blocks for reasoning, then produces tool calls. The thinking content is separated from the tool call output.
...
|----------|-------------|-------------|
| `prime` | Prime infrastructure | Provider-specific |
| `modal` | Cloud-based sandboxes with fast cold starts | Modal SDK auth |
| `daytona` | Self-hosted sandbox environments | `DAYTONA_API_KEY`, `DAYTONA_API_URL`, `DAYTONA_TARGET` env vars |
| `e2b` | Cloud sandboxes for prototyping and development | `E2B_API_KEY` env var |

### Using sandboxes in environments

The `mini_swe_agent_plus` environment demonstrates the full pattern:
