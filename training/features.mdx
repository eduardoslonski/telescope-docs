...
vLLM inference servers support tool calling parsers (e.g., Hermes) for training agentic models that invoke tools during rollouts. Tool calls are parsed from model output, executed by the environment, and results are fed back as the next turn. Controlled by `enable_tool_call` and `tool_call_parser`. See [Tool Calling & Agentic Training](/training/tool-calling).
...
When training on multiple environments with different reward scales, `reward_min` and `reward_max` per environment allow Telescope to normalize rewards to a common range. This prevents environments with larger reward magnitudes from dominating the training signal. See [Reward Design](/training/reward-design).
